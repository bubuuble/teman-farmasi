...
export default function Hero({ content }: HeroProps) {
  ...
  return (
    <section className="relative pt-28 pb-20 lg:pt-40 lg:pb-32 overflow-hidden bg-brand-cream">
      ...
      {/* 1. Large Blurred Blobs (Existing) */}
      <div className="absolute top-0 right-0 w-[600px] h-[600px] bg-brand-pink/5 rounded-full blur-3xl -z-10"></div>
      <div className="absolute bottom-0 left-0 w-[500px] h-[500px] bg-brand-pink rounded-full blur-3xl -z-10"></div>

      {/* 2. Abstract Molecular Structure (Samar di kiri bawah) */}
      <div className="absolute bottom-20 left-10 text-brand-darkblue -z-10 hidden lg:block rotate-12">
        <svg width="400" height="400" viewBox="0 0 200 200" fill="none" stroke="currentColor" strokeWidth="2">
            <circle cx="40" cy="40" r="8" />
            <circle cx="100" cy="40" r="8" />
            <circle cx="130" cy="90" r="8" />
            <circle cx="100" cy="140" r="8" />
            <path d="M48 40 H92 M108 40 L124 82 M124 98 L108 132 M100 48 V132" />
        </svg>
      </div>

      {/* 3. Floating Pharmacy Icons (Elemen melayang samar) */}
      <div className="absolute top-1/4 left-[5%] text-brand-pink opacity-50 animate-float hidden lg:block">
        <GiMedicines size={80} />
      </div>
      <div className="absolute bottom-1/4 right-[6%] text-brand-blue opacity-40 animate-float-delayed hidden lg:block">
        <GiDna2 size={100} />
      </div>
      <div className="absolute top-1/3 right-[45%] text-brand-blue animate-pulse hidden lg:block">
        <GiChemicalDrop size={40} />
      </div>
      <div className="absolute bottom-1/3 left-[40%] text-brand-pink opacity-30 animate-float hidden lg:block">
        <GiChemicalTank size={50} />
      </div>
      <div className="absolute top-1/2 left-[5%] text-brand-blue opacity-20 animate-float-delayed hidden lg:block">
        <GiMedicinePills size={60} />
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-8 items-center relative z-10">
          ...
          <div className="relative order-1 lg:order-2 h-[500px] lg:h-[650px] w-full flex items-center justify-center">
             ...
             <div className="relative w-full max-w-[500px] aspect-square">
                ...
                {/* Visual Layers behind image */}
                <div className="absolute top-0 right-0 w-[350px] h-[350px] bg-brand-yellow rounded-full -z-10 animate-pulse"></div>
                <div className="absolute bottom-10 left-4 w-[300px] h-[300px] bg-brand-pink rounded-[3rem] -rotate-12 -z-10 opacity-80"></div>

                {/* Main Image */}
                <div className="absolute inset-4 z-10 rounded-[3rem] overflow-hidden border-8 border-white shadow-2xl bg-white transform rotate-2 hover:rotate-0 transition-transform duration-500">
                    <Image 
                        src={mainImageUrl}
                ...
                {/* Floating Badge 1 */}
                <div className="absolute top-10 -right-4 z-20 bg-brand-blue p-4 rounded-2xl shadow-xl border-l-4 border-white flex items-center gap-3 animate-float">
                      <div className="bg-white/20 p-2.5 rounded-full text-white"><FaStar size={16} /></div>
                      <div>
                ...
                {/* Floating Badge 2 */}
                <div className="absolute bottom-16 -left-6 z-20 bg-brand-yellow p-4 rounded-2xl shadow-xl border-l-4 border-white flex items-center gap-3 animate-float-delayed">
                      <div className="bg-white/20 p-2.5 rounded-full text-white"><FaUserGraduate size={16} /></div>
                      <div>
                ...
                {/* Decorative Icon: Beaker & Microscope */}
                <div className="absolute -bottom-8 right-10 z-20 bg-brand-pink text-white w-16 h-16 rounded-2xl flex items-center justify-center text-2xl shadow-lg transform rotate-12 hover:rotate-0 transition-transform">
                    <FaFlask />
                </div>
                <div className="absolute -top-4 left-10 z-20 bg-brand-darkblue text-white w-14 h-14 rounded-2xl flex items-center justify-center text-xl shadow-lg transform -rotate-12 hover:rotate-0 transition-transform">
                    <FaMicroscope />
                </div>
