...
type EnrolledClass = {
  ...
  classes: {
    ...
    // Perbaikan struktur tipe join mentor
    class_mentors: {
...
export default async function StudentClassesPage() {
  ...
  // Query diperbaiki untuk deep join
    ...
    <div className="space-y-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {enrollments?.map((item) => {
          ...
          // Logic pengambilan nama mentor yang lebih aman
          const mentorName = kelas.class_mentors?.[0]?.profiles?.full_name || "Tim Pengajar"

          return (
            <div key={kelas.id} className="bg-white rounded-[40px] p-8 shadow-sm hover:shadow-xl transition-all border border-gray-100 group flex flex-col h-full hover:-translate-y-1">
